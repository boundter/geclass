...
if [ -f /app/nginx.conf ]; then
    ...
    content_server=$content_server'    }\n'
    content_server=$content_server'    location /report/ {\n'
    content_server=$content_server'        proxy_pass http://localhost:5000/report/;\n'
    content_server=$content_server'    }\n'
...
printenv | grep 'QUAMP_USER' >> /etc/environment
printenv | grep 'QUAMP_PASSWD' >> /etc/environment
